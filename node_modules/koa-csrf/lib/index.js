...
var CSRF = function CSRF(opts) {
  ...
  this.middleware = function (ctx, next) {
    ...
    if (_this.opts.excludedMethods.includes(ctx.method)) return next();
    ...
    if (!token) return ctx.throw(_this.opts.invalidTokenMessage, _this.opts.invalidTokenStatusCode);
